...
public class GetCurrentPollsQueryHandler(IPollRepository pollRepository)
    : IRequestHandler<GetCurrentPollsQuery, Result<IEnumerable<PollResponse>>>
{
    public async Task<Result<IEnumerable<PollResponse>>> Handle(GetCurrentPollsQuery request, CancellationToken cancellationToken)
    {
